...
public class PlayerCharacter : MonoBehaviour
{
	...
	void UpdatePlatformMovement()
	{
		if (currentPlatform != null)
		{
			...
			transform.position += platformDelta; // Move o jogador junto com a plataforma
	...
	private void OnTriggerEnter2D(Collider2D other)
	{
		...
		if (other.CompareTag(Tags.MOVING_PLATFORM))
		{
			Debug.Log("Entrou");
			if (isGrounded)
				Debug.Log("Grounded");
	...
	private void OnTriggerExit2D(Collider2D other)
	{
		...
		if (other.CompareTag(Tags.MOVING_PLATFORM) && currentPlatform != null)
		{
